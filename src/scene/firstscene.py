...
class FirstScene(SceneObject):
    ...
    def __init__(self, player: Player, enemies: List[GameObject], game_state: GameState):
        ...
        self._game_state.set_objects_in_scene(self.SCENE_TYPE, [self.player] + enemies)
    ...
    def update(self) -> Scene | None:
        ...
        foo = self._game_state.objects_in_scene(self.SCENE_TYPE)
        for game_object in self._game_state.objects_in_scene(Scene.FIRST_LEVEL):
            #2: handle input and construct an action from there
            action = game_object.get_action(self._game_state)

            if action:
                #3: check if the action is valid - if it is, apply that action
                self._action_manager.parse_action(game_object, action, Scene.FIRST_LEVEL)
